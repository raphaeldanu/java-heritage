...
<x-adminlte-card theme="teal" theme-mode="outline">
  <form action="{{ route('training-menus.store') }}" method="POST">
    @csrf
    <x-adminlte-input name="title" label="Training Menu Title" type="text" id="name" placeholder="Training Menu Title" enable-old-support/>
    <x-adminlte-select2 name="training_subject_id" label="Training Subject" enable-old-support>
      <x-adminlte-options empty-option="Select Training Subject" :options="$subjects"/>
    </x-adminlte-select2>
    <x-adminlte-select2 name="department_id" label="Department (leave it empty if not departmental training)" enable-old-support>
      <x-adminlte-options empty-option="Select Department" :options="$departments"/>
    </x-adminlte-select2>
