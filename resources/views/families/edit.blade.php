...
<x-adminlte-card title="Add Family Member For {{ $employee->name }}" theme="teal" theme-mode="outline">
  <form action="@if ($breadcrumb == 'employee') {{ route('families.update', ['employee' => $employee, 'family' => $family]) }} @elseif ($breadcrumb == 'my-data') {{ route('my-data.update-family', ['family' => $family]) }} @endif" method="POST">
    @csrf
    @method('PUT')
    <x-adminlte-input name="name" placeholder="Name" label="Name" value="{{ $family->name }}" enable-old-support/>
    @php
      $selected = old('relationship', $family->relationship->value)
    @endphp
