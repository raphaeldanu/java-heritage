...
<x-adminlte-card theme="teal" theme-mode="outline">
  <form action="{{ route('positions.update', ['position' => $position]) }}" method="POST">
    @csrf
    @method('put')
    <x-adminlte-input name="name" label="Position Name" type="text" id="name" placeholder="Position Name" enable-old-support value="{{ $position->name }}"/>
    @php
      $options = $departments->mapWithKeys( fn($item, $key) => [$item['id'] => $item['name']] )->all();
      $selected = [$position->department_id];
    @endphp
    <x-adminlte-select2 name="department_id" label="Department" enable-old-support>
      <x-adminlte-options empty-option="Select department" :options="$options" :selected="$selected"/>
    </x-adminlte-select2>
