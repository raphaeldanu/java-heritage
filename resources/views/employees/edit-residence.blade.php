...
<x-adminlte-card title="Edit Residence Address For {{ $employee->name }}" theme="teal" theme-mode="outline">
  <form action="@if ($breadcrumb == 'employee') {{ route('employees.update-residence', ['employee' => $employee]) }} @elseif ($breadcrumb == 'my-data') {{ route('my-data.update-residence') }} @endif" method="POST">
    @csrf
    @method('put')
