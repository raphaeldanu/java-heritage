...
<x-adminlte-card theme="teal" theme-mode="outline">
  ...
  <form action="{{ route('approve-leave-requests.update', ['leave_request' => $leaveRequest]) }}" method="POST">
    @csrf
    @method('PUT')
    ...
    <x-adminlte-select2 name="status" label="Status" enable-old-support>
      <x-adminlte-options empty-option="Select Status" :options="$statuses" :selected="$leaveRequest->status->value"/>
    </x-adminlte-select2>
    <x-adminlte-textarea name="note_from_approver" placeholder="Note From Approver" label="Note" rows=3 enable-old-support>
      {{ $leaveRequest->note_from_approver }}
    </x-adminlte-textarea>
